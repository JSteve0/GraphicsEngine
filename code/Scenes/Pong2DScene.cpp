...
#include "Pong2DScene.hpp"

Pong2DScene::Pong2DScene (OpenGLContext* context, ShaderProgram* colorShaderProgram, ShaderProgram* normalShaderProgram) 
  : Scene::Scene(normalShaderProgram)
{
  ...
  m_player2 = new AI(opponent, size, 5.0f);
  m_ball = new Ball(ball, Vector3(0.4f, 0.4f, 0.0f), Vector3(6.0f, 3.0f, 0.0f));

  m_gamestate = 0;
}

void
Pong2DScene::update(float deltaTime)
{
  if (m_gamestate != 0)
  {
    ...
    m_player2->update(m_ball, deltaTime, -5.0f, 5.0f, 0.0f, 0.0f);
    if ((m_ball->collides(m_player) && m_ball->getVelocity().m_x < 0)|| (m_ball->collides(m_player2) && m_ball->getVelocity().m_x > 0))
    {
      m_ball->bounce();
    }
  } else {
...
void 
Pong2DScene::processKeys (KeyBuffer* keyBuffer, float deltaTime)
{
...
void
Pong2DScene::resetCamera(Camera* camera)
{
